...
def _write_proc_registry_unlocked(data: dict[str, dict]) -> None:
    _PROC_REGISTRY_PATH.parent.mkdir(parents=True, exist_ok=True, mode=0o700)
    tmp = _PROC_REGISTRY_PATH.with_suffix(".tmp")
    # Write with mode 0o600 so the registry (which contains PIDs) is only
    # readable by the owning user on multi-user systems.
...
def _sse_smart_skip_process(
    base_cmd: list[str],
    roots: list[Path],
    detect_bpm: bool,
    detect_key: bool,
    library_root: str = "",
    cleanup_on_done: list[Path] | None = None,
) -> Response:
    """
...
def _smart_skip_candidates(roots: list[Path], detect_bpm: bool, detect_key: bool) -> dict:
    ...
    for root in roots:
        ...
        for track in scan_directory(root):
            total += 1
            # Use bpm/key already read by extract_metadata — avoids a second
            # MutagenFile open per file (halves I/O on the smart-skip pre-scan).
            # track.bpm/key are None when the tag is absent, unparseable, or
            # the file errored — all three cases correctly trigger processing.
            needs_bpm = detect_bpm and track.bpm is None
            needs_key = detect_key and track.key is None
            if needs_bpm or needs_key:
                pending.append(str(track.path))
            else:
                skipped_complete += 1
            if track.errors:
                unreadable += 1
