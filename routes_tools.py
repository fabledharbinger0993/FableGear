...
def _load_duplicate_cache(csv_path: Path, *, include_db: bool = True) -> dict:
    """Load and cache a duplicate report CSV.

    Thread-safe: the cache dict is only read or written while holding
    _report_cache_lock.  The expensive CSV + DB load runs outside the lock
    so it does not block concurrent requests for different reports; a
    duplicate load for the same key on cache miss is harmless (last writer
    wins, both produce identical results from the same file mtime).
    """
    cache_key = str(csv_path.resolve())
    csv_mtime = csv_path.stat().st_mtime

    # Fast path: return cached entry if it is still fresh.
    ...
    # Slow path: parse CSV and (optionally) enrich with DB data.
...
def api_run_prune():
    ...
    # Check and increment atomically inside one lock acquire — without this
    # two concurrent requests can both read _active_prune_workers == 0 and
    # both proceed past the guard (TOCTOU race).
    with _prune_state_lock:
        global _active_prune_workers
        ...
        _PRUNE_CANCEL_EVENT.clear()  # reset inside the same lock so no prune slip through
