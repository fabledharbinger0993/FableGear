...
run_once() {
  ...
  if [[ ! -f "$REPO_ROOT/scripts/rekit_agent.py" ]]; then
    log "missing dependency: $REPO_ROOT/scripts/rekit_agent.py"
    log "status/start/stop are available; once/watch require rekit_agent.py"
    return 1
  fi
