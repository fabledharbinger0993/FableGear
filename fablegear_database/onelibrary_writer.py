...
def _file_type_code(fmt: str | None) -> int | None:
    if not fmt:
        return None
...
def _safe_segment(name: str | None, fallback: str) -> str:
    seg = (name or "").strip().translate(_UNSAFE_SEGMENT).strip(". ")
    return seg[:120] or fallback


def _stage_audio_file(track, drive_root: Path) -> str | None:
...
class OneLibraryWriter:
        ...
        try:
            ...
            for track in sorted(tracks, key=lambda t: t.id or 0):
                try:
                    ...
                    bpmx100 = round(track.bpm * 100) if track.bpm else None
                    length = round(track.duration) if track.duration else None
    ...
    def _write_playlists(
        ...
        def _walk(nodes: list[dict], parent_id: int | None) -> None:
