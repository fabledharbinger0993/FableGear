...
_TRANSACTION_ID_RE = re.compile(r"^\d{8}_\d{6}_\d+$")
...
def undo_database_revert():
    ...
    # Reject IDs that don't match the expected YYYYMMDD_HHMMSS_microseconds format
    # before they reach the undo manager — prevents path-like or SQL-like strings.
    if not _TRANSACTION_ID_RE.match(str(transaction_id)):
    ...
    try:
        ...
        if success:
            log.info("Database transaction reverted: %s", transaction_id)
