...
def api_brew_upgrade():
    """SSE stream of ``brew upgrade <packages>`` for known-outdated FableGear packages.

    Brew is an external binary and must NOT be passed through _sse_response() /
    _stream(), which enforce a Python-only subprocess policy. Instead we stream
    directly from a Popen here, gated against the BREW_DEPS allowlist so no
    package name from the status cache can be used to upgrade an arbitrary formula.
    """
    from brew_updater import BREW_DEPS  # noqa: PLC0415

    outdated = _brew_get_status().get("outdated", [])
    # Intersect against the compile-time allowlist — never trust the cached name alone.
    names = [
        p["name"] for p in outdated
        if p.get("name") and p["name"] in BREW_DEPS
    ]

    def _gen(packages: list[str]):
        if not packages:
            yield f"data: {json.dumps({'line': 'No outdated FableGear packages found.'})}\n\n"
        ...
        try:
            ...
            yield f"data: {json.dumps({'line': '[ERROR] brew not found — is Homebrew installed?'})}\n\n"
            yield f"data: {json.dumps({'done': True, 'exit_code': 1})}\n\n"
            yield ": end\n\n"
            return
        except Exception as exc:
            yield f"data: {json.dumps({'line': f'[ERROR] Could not start brew: {exc}'})}\n\n"
            yield f"data: {json.dumps({'done': True, 'exit_code': 1})}\n\n"
            yield ": end\n\n"
            return
...
def api_onboarding_install_app():
    ...
    # Use mkstemp so the file is created atomically — mktemp() is deprecated
    # and carries a TOCTOU race between name generation and first open.
    script_content = f'do shell script "bash \'{launch_sh}\' > /dev/null 2>&1 &"'
    _fd, _tmp_str = tempfile.mkstemp(suffix=".applescript")
    tmp_as = Path(_tmp_str)
    try:
...
_OB_IMPORT: dict = {"running": False, "phase": "idle", "done": 0, "total": 0,
                    "result": None, "error": None}
_OB_IMPORT_LOCK: threading.Lock = threading.Lock()


def _ob_import_snapshot() -> dict:
    """Return a shallow copy of _OB_IMPORT under the lock (safe for JSON serialisation)."""
    with _OB_IMPORT_LOCK:
        return dict(_OB_IMPORT)


def _ob_import_update(**kwargs) -> None:
    """Thread-safe bulk update of _OB_IMPORT fields."""
    with _OB_IMPORT_LOCK:
        _OB_IMPORT.update(kwargs)
...
def api_onboarding_import_sources():
    ...
    # Check-and-set inside a single lock acquire to prevent the TOCTOU race
    # where two simultaneous requests both see running=False and both start.
    with _OB_IMPORT_LOCK:
    ...
    def _run():
        try:
            ...
            def _progress(done, total):
                _ob_import_update(phase="importing", done=done, total=total)

            db = FableGearDatabase()
            result = FileImporter(db).import_files(roots, progress_callback=_progress)
            _ob_import_update(result=result)
        except Exception as exc:  # noqa: BLE001 — surfaced to the wizard UI
            _ob_import_update(error=str(exc))
        finally:
            _ob_import_update(running=False, phase="done")
...
def api_onboarding_import_sources_status():
    return jsonify(_ob_import_snapshot())
